...
//?  9，MVVM,MVC
/*
        MVC:(模型，视图，控制器)，view传指令到controller完成业务逻辑后要求model更变状态再将数据发送到view更新视图得到反馈
        9，MVVM:(模型，视图，视图模型)，最大特点是双向绑定，实现了view通过viewmodel对model自动同步，或者反向同步
*/
//?  10，递归
/*
        函数内部直接或间接调用函数本身即为递归
...
//?  24，虚拟DOM
/*
        就是用js对象来描述真实的dom，因为直接操作dom性能开销会比较大，
        而通过js来操作这个虚拟dom再通过diff算法来对比差异，进行针对性的更新dom会很高效。

        diff算法的作用是对比新旧虚拟dom差异 记录下来。然后DOM fragment 统一计算出所有变化后统一更新一次dom

        虚拟dom的意义：性能不错的更新策略。实现跨平台，服务端渲染
...
//?  86，继承：
/*
        1，原型链继承:通过在构造函数B的prototype等于new出来的A的实例化 B.prototype = new A()，
        然后再new B 那么B的实例化对象就拥有了构造函数A的属性，相当于构造函数B继承了构造函数A，再new B
        缺陷就是多个实例所继承到的属性如果是复杂数据类型则改一个实例的属性会导致全部实例的该属性都会改变。
        （属性共享）

        2，构造函数继承：
        构造函数B内部使用A.call(this)绑定继承A的属性方法。new B 时就可以使用A的属性了，
        某实例改变属性不会影响其他实例，（属性不再共享）。

        3，组合继承：
        缺点是new了两次，需要实例化两次性能不太好，组合继承就是原型链继承和构造函数继承的组合

        4，寄生组合继承：
        与组合继承相同，但是没有再次new A
        B.prototype = Object.create(A.prototype)
        B的prototype不会影响A的prototype

        5,class
        定义类,内部必须要定义构造函数constructor
        继承时需要使用 extends 例:  class B extends A{} B继承A  B的constructor内再使用super()
