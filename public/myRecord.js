...
/*
        ...
        MVC：view发送指令，controller接收处理，要求model更改数据，然后model因为观察者模式，会通知view更改视图。

        MVVM：是分离view跟model，所有操作都经过viewModel，Model和ViewModel之间有着双向数据绑定的联系
*/
//?  10，递归
/*
        函数内部直接或间接调用函数本身即为递归
...
/*
        ...
        虚拟DOM也为跨平台开发提供了极大的便利，开发者写的同一套代码，通过不同的渲染规则，就可以生成不同平台的代码。
...
/*
        ...
        将创建的空对象原型（__proto__）指向构造函数的prototype属性，
...
/*
        基本数据类型的赋值都是深拷贝，
        在引用类型的数据中实现浅拷贝可以使用
        es6扩展运算符 Array.from方法 concat()方法 slice()方法,object.assign()方法
...
/*
        ...
        Promise.allSettled（set熬的）  需要执行多个平行和独立的异步操作并收集所有结果时非常有效，即使某些异步操作可能失败
                与 all()的区别：
...
/*
        ...
        总结: 模块化思想，都是引用模块的方式。模块化规范不同，写法不同。
              require是赋值过程，他的结果就是对象函数等，再把require的结果赋值给某个变量。
              import是解构过程，export用于对外输出本模块变量的接口，import加载引用。
*/
//?  43，原型和原型链
/*
        隐式原型:__proto__
...
//?  59，判断类型的方法
/*
        1，typeof: 它是一元运算符，返回全小写字符串，null、[]等返回object。例: typeof [], null; //object  其他都是正常类型
        2，instanceof: 通过原型链判断A是否为B的实例。 例: [] instanceof Array;  //true 只能判断引用类型，函数 对象 数组三个
        3，constructor: 通过原型链遗传的类型，创建的值的类型被遗传了。 例: ''.constructor == string //true 各种类型都能判断
...
/*
        ...
        提供了几种钩子函数: bind  绑定元素时调用，执行一次
                        inserted（in色提的）  被绑定的元素，插入到父节点的dom时调用
...
*/
//?  110，typeof和instanceof的区别
/*
        typeof能识别所有的值类型，识别函数，能区分是否是引用类型。 null与array都为object
        instanceof用于检测构造函数的 prototype 属性是否出现在某个实例对象的原型链上。

*/
//?  111，TypeScript type和interface的区别
/*
        interface可以重复声明，type不行，
        继承方式不一样，type使用交叉类型方式，interface使用extends实现。
        在对象扩展的情况下，使用接口继承要比交叉类型的性能更好。
        建议使用interface来描述对象对外暴露的接口，使用type将一组类型重命名（或对类型进行复杂编程）。
*/
//?  111，TypeScript any、unkonwn、never
/*
        any和unkonwn在TS类型中属于最顶层的Top Type，即所有的类型都是它俩的子类型。
        而never则相反，它作为Bottom Type是所有类型的子类型。
*/
//?  112，WeakMap和Map之间的区别
/*
        WeakMap只能以复杂数据类型作为key，并且key值是弱引用，对于垃圾回收更加友好，因为引用计数的回收方式不会计算weakMap的引用。
*/
//?  113，HTML、XHTML和HTML5区别以及有什么联系
/*
        XHTML是HTML规范版本
...
/*
        ...
        防止不必要的安全问题
*/
//?  116，Number.isNaN与isNaN
/*
        isNaN 会先将值转换为number类型，转换失败则为true
        Number.isNaN 不存在类型转换的行为, 只有值为NaN时才返回true
*/
//?  117，引用文件时经常用到的@符，一般指向的是哪
/*
        根目录路径  也就是src
*/
//?  118， npm run dev执行过程
/*
        检查环境 -> 启动服务器 -> 编译代码 -> 监听文件变化 -> 自动刷新页面。
*/
//?  119，前端工程化
/*
